...
if [ -f /app/.env ]; then
    echo "P12 Relatorios: .env em /app/.env"
else
    ...
    if [ -s /app/.env ]; then
        echo "P12 Relatorios: .env gerado a partir do ambiente Easypanel"
    else
        echo "P12 Relatorios: aviso — nenhuma variável reconhecida para .env"
...
echo "P12 Relatorios: webhook Meta Leads :${WEBHOOK_PORT} (stdout + .tmp/webhook_meta_leads.log; filtro P12_META_LEAD_WEBHOOK)"
...
if [ "${ENABLE_DASHBOARD}" = "true" ] || [ "${ENABLE_DASHBOARD}" = "1" ] || [ "${ENABLE_DASHBOARD}" = "yes" ]; then
    ...
    echo "P12 Relatorios: dashboard :${DASHBOARD_PORT} (stdout + .tmp/dashboard.log)"
else
    echo "P12 Relatorios: dashboard desligada (ENABLE_DASHBOARD=${ENABLE_DASHBOARD})"
fi
...
echo "P12 Relatorios: cron segundas 10:00 (TZ do container) — job .tmp/cron.log · crond .tmp/crond.log"
